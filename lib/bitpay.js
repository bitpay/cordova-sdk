 ...
 */
Bitpay.prototype.openInvoice = function(invoice){
  var self = this;
  self.isWalletAvailable(function(available) {
    return available ?  self.openWallet(invoice) : self.openBrowser(invoice);
  });
}
 ...
 */
Bitpay.prototype.openWallet = function(invoice){
  window.open(invoice.data.paymentUrls.BIP72, '_system');
}

/**
 * Open an invoice in a innapp browser
 */
Bitpay.prototype.openBrowser = function(invoice){
  window.open(invoice.data.paymentUrls.BIP73, '_system');
}
