...
BitpayInvoice.prototype.subscribePaymentEvent = function(){
  ...
  var openBusAndSubscribe = function(err, response){
    ...
    self.bus.addEventListener('statechange', function(event){
      ...
      if ( invoice.status == 'new' || invoice.status == 'paid' || invoice.status == 'confirmed' ) {
        self.data = invoice;
...
/**
 * Will open up the users bitcoin wallet or display the invoice on the browser
 */
BitpayInvoice.prototype.open = function(){
  var self = this;
  var Bitpay = cordova.require('com.bitpay.sdk.cordova.Bitpay');
  Bitpay.isWalletAvailable(function(available) {
    return available ?  self.openWallet() : self.openBrowser();
  });
}
 ...
 */
BitpayInvoice.prototype.openWallet = function(){
  window.open(this.data.paymentUrls.BIP72, '_system');
}

/**
 * Open an invoice in a innapp browser
 */
BitpayInvoice.prototype.openBrowser = function(){
  window.open(this.data.paymentUrls.BIP73, '_system');
}
